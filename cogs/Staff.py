...
class Staff(commands.Cog):
    ...
    def is_manager(ctx):
        ...
        # Allowed Roles List (change to your server roles)
    ...
    @commands.command()
    @commands.check(is_manager)
    async def unbind(self, ctx, target: discord.User):
        print(f"Unbind command invoked by {ctx.author} for {target}")
        result = await Db_Handler.unbind(target.id)
